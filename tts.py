...
hps = ttsUtils.get_hparams_from_file(args.cfg)
...
# 播放语音和更改文字
async def play(isRun,ttsQue,ttsConfig,currTXT):
    while isRun:
        name = ttsQue.get()
        playsound('output/'+name+'Send.wav')
        playsound('output/'+name+'Rec.wav')
        await asyncio.sleep(0.5)

# 生成语音
def generated_speech(text, fileName,type):
    if type=="cpu":
        cpu_tts(text, fileName)
    else:
        print("2")
