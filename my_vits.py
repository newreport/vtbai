...
def get_args():
    ...
    parser.add_argument('--onnx2_model', default = './models/G_latest.onnx')
    # 模型配置
    parser.add_argument('--onnx2cfg', default="./models/config.json")
    parser.add_argument('--cfg', default="./models/moegoe_config.json")
    # 输出目录
...
def generated_speech2(text,fileName):
    text = f"[JA]{text}[JA]" if is_japanese(text) else f"[ZH]{text}[ZH]"
    # 将文本字符串转换为id
    seq = text_to_sequence(text, symbols=hps.symbols, cleaner_names=hps.data.text_cleaners)
    if hps.data.add_blank:
...
if __name__ == '__main__':
    ...
    #  generated_speech(str1,'temp1.wav')
     generated_speech2(str1,'temp1.wav')